...
export default function LegacyMetalQuoteApp() {
  // Header fields
    ...
    try {
      ...
      img.onload = () => {
        try {
          ...
          console.warn("Logo toDataURL failed:", e?.message);
        }
      };
      img.onerror = () => console.warn("Logo failed to load from", LOGO_SRC);
      img.src = LOGO_SRC;
    } catch (e) {
      console.warn("Logo preload error:", e?.message);
    }
  }, []);
  ...
  // Pricing modifiers
  const markupPct = 33; // fixed markup percentage
  ...
  const calcForGauge = (gauge) => {
    ...
    const factor = 1 + toNum(markupPct) / 100;

    // Panels
    const panelUnit = PRICES.panelLF[gauge] * factor;
    const panelCost = panelLF * panelUnit;
    ...
    const trimCost = Object.entries(pieces).reduce(
      (sum, [key, qty]) => sum + qty * ((trimPrices[key] ?? 0) * factor),
      0
    );
    ...
    const iwsCost = iwsQty * (iws.price * factor);
    ...
    const taxableBase = subtotal; // markup already applied per item
    ...
    return {
      ...
      panelLF,
      lines: [
        { label: `Panels (${panelLF.toFixed(0)} lf)`, qty: panelLF, unit: "lf", price: panelUnit, total: panelCost },
        { label: `Hip`, qty: pieces.hip, unit: "10' pcs", price: trimPrices.hip * factor, total: pieces.hip * (trimPrices.hip * factor) },
        { label: `Ridge`, qty: pieces.ridge, unit: "10' pcs", price: trimPrices.ridge * factor, total: pieces.ridge * (trimPrices.ridge * factor) },
        { label: `Gable Rake`, qty: pieces.gable, unit: "10' pcs", price: trimPrices.gable * factor, total: pieces.gable * (trimPrices.gable * factor) },
        { label: `Drip Edge`, qty: pieces.drip, unit: "10' pcs", price: trimPrices.drip * factor, total: pieces.drip * (trimPrices.drip * factor) },
        { label: `Valleys`, qty: pieces.svalley, unit: "10' pcs", price: trimPrices.svalley * factor, total: pieces.svalley * (trimPrices.svalley * factor) },
        { label: `Sidewall`, qty: pieces.sidewall, unit: "10' pcs", price: trimPrices.sidewall * factor, total: pieces.sidewall * (trimPrices.sidewall * factor) },
        { label: `Endwall`, qty: pieces.endwall, unit: "10' pcs", price: trimPrices.endwall * factor, total: pieces.endwall * (trimPrices.endwall * factor) },
        { label: `Transition`, qty: pieces.transition, unit: "10' pcs", price: trimPrices.transition * factor, total: pieces.transition * ((trimPrices.transition ?? 0) * factor) },
        { label: `${iws.label}`, qty: iwsQty, unit: "rolls", price: iws.price * factor, total: iwsCost },
        { label: `Panel Clips`, qty: clipBoxes, unit: "box", price: PRICES.clips.pricePerBox * factor, total: clipCost },
        { label: `Pancake Screws`, qty: screwBags, unit: "bag", price: PRICES.screws.pricePerBag * factor, total: screwCost },
        { label: `Z Metal`, qty: zQty, unit: "10' pcs", price: PRICES.z[gauge] * factor, total: zCost },
        { label: `Perforated Z Metal`, qty: zPerfQty, unit: "10' pcs", price: PRICES.zPerf[gauge] * factor, total: zPerfCost },
        { label: `Staple Pack`, qty: stapleBoxes, unit: "box", price: PRICES.staples.pricePerBox * factor, total: stapleCost },
      ],
      subtotal,
      taxableBase,
      taxAmt,
  ...
  const result24 = useMemo(() => calcForGauge("24"), [inputs, iwsChoice]);
  const result26 = useMemo(() => calcForGauge("26"), [inputs, iwsChoice]);

  // --- Minimal self-tests (console) ---
  useEffect(() => {
    const approx = (a, b, eps = 1e-4) => Math.abs(a - b) < eps;
    // Coverage & rounding helpers
    ...
    // Filename
    console.assert(
      buildFileName("ACME Roofing", "PO123", "2025-08-19") ===
        "ACME_Roofing-PO123-2025-08-19.pdf",
      "filename format"
    );
    console.assert(
      buildFileName("", "", "2025-08-19").startsWith("Quote-"),
      "fallback filename format"
    );
    // IWS stock flags
    console.assert(PRICES.iws.standard.label === "Polyglass", "iws label Polyglass");
    console.assert(PRICES.iws.standard.stock === "Out of Stock", "Polyglass OOS flag");
    console.assert(PRICES.iws.butyl.label === "GripRite", "iws label GripRite");
    console.assert(PRICES.iws.butyl.stock === "In Stock", "GripRite in-stock flag");
    // Markup factor sanity (unit price increases)
    const f = 1 + 33 / 100;
    console.assert(PRICES.panelLF["24"] * f > PRICES.panelLF["24"], "markup raises panel unit");
    console.assert(PRICES.trim24.hip * f > PRICES.trim24.hip, "markup raises trim unit");
  }, []);
  ...
  const buildPdfDoc = () => {
    ...
    const left = 40,
      ...
      bottom = 48,
      colGap = 40,
      colWidth = (pageWidth - left * 2 - colGap) / 2;

    // Header logo + title (preserve aspect ratio)
    ...
    // Title
    doc.setFontSize(16);
    doc.text("Legacy Metal Quote", left, titleY);
    ...
    if (customer) {
      doc.text(`Customer: ${customer}`, left, headerY);
      headerY += 14;
    }
    if (po) {
      doc.text(`PO: ${po}`, left, headerY);
      headerY += 14;
    }
    // Then Notes right under PO
    if (notes && notes.trim()) {
      doc.setFontSize(11);
      doc.text("Notes:", left, headerY);
      headerY += 14;
      doc.setFontSize(10);
      const wrappedHead = doc.splitTextToSize(notes, pageWidth - left * 2);
      wrappedHead.forEach((line) => {
        doc.text(line, left, headerY);
        headerY += 12;
      });
    }
    ...
    const drawCol = (x, title, res) => {
      ...
      y += 16;
      doc.setFontSize(10);
      doc.text(`Panels: ${res.panelLF.toFixed(0)} lf`, x, y);
      y += 14;
      doc.text("Items:", x, y);
      y += 12;
      ...
      lines.forEach((ln) => {
        const qtyStr = `${ln.qty % 1 === 0 ? ln.qty : Math.round(ln.qty)} ${
          ln.unit || ""
        }`;
      ...
      y += 12;
      doc.text(`Subtotal: ${fmt(res.subtotal || 0)}`, x, y);
      y += 12;
      doc.text(`Tax (${taxPct}%): ${fmt(res.taxAmt || 0)}`, x, y);
      y += 12;
      doc.setFontSize(12);
      doc.text(`Grand Total: ${fmt(res.grandTotal || 0)}`, x, y);
    };
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      doc.setFontSize(9);
      doc.text(
        "Powered by Empire Supply - 801-391-7549",
        pageWidth / 2,
        ph - 24,
        { align: "center" }
      );
  ...
  const previewPDF = () => {
    try {
      ...
      try {
        ...
        console.warn("Blob preview failed, trying data URI:", err?.message);
      }
      try {
        ...
        console.warn("Data URI preview failed; forcing download:", err?.message);
        exportPDF();
      }
    } catch (e) {
    ...
    <div className="p-4 max-w-5xl mx-auto">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-base font-bold flex items-center gap-2">
          <FileText className="w-5 h-5" /> Legacy Metal Quote
          <span className="ml-2 text-slate-500">v{APP_VERSION}</span>
        </h2>
      </div>
      ...
      <div className="mt-4 mb-6 bg-white border border-slate-200 rounded-2xl overflow-hidden ring-1 ring-slate-200 shadow-sm">
        <div className="px-3 py-2 text-base font-bold uppercase bg-slate-50 border-b">
          Job Details
        </div>
        <div className="px-3 divide-y">
          <div className="flex items-center justify-between gap-3 py-2">
            <label htmlFor="field-customer" className="text-sm font-medium whitespace-nowrap">
              Customer
            </label>
            <input
              id="field-customer"
          ...
          <div className="flex items-center justify-between gap-3 py-2">
            <label htmlFor="field-po" className="text-sm font-medium whitespace-nowrap">
              PO
            </label>
            <input
              id="field-po"
      ...
      <div className="mt-0 bg-white border border-slate-200 rounded-2xl overflow-hidden ring-1 ring-slate-200 shadow-sm">
        ...
        <div className="px-3 divide-y">
          <div className="flex items-center justify-between gap-3 py-2">
            <label htmlFor="field-sqft" className="text-sm font-medium whitespace-nowrap">
              Total Square Feet
            </label>
            <input
              id="field-sqft"
          ...
          <div className="flex items-center justify-between gap-3 py-2">
            <label htmlFor="field-iws" className="text-sm font-medium whitespace-nowrap">
              Type of Ice & Water
            </label>
            <select
              id="field-iws"
            ...
            >
              {Object.entries(PRICES.iws).map(([key, item]) => (
                <option key={key} value={key}>
                  {item.label} ({item.stock || "In Stock"})
                </option>
              ))}
            </select>
          </div>
          <div className="flex items-center justify-between gap-3 py-2">
            <label htmlFor="field-hips" className="text-sm font-medium whitespace-nowrap">
              Hips (lf)
            </label>
            <input
              id="field-hips"
          ...
          <div className="flex items-center justify-between gap-3 py-2">
            <label htmlFor="field-ridges" className="text-sm font-medium whitespace-nowrap">
              Ridges (lf)
            </label>
            <input
              id="field-ridges"
          ...
          <div className="flex items-center justify-between gap-3 py-2">
            <label htmlFor="field-gables" className="text-sm font-medium whitespace-nowrap">
              Gable Rakes (lf)
            </label>
            <input
              id="field-gables"
          ...
          <div className="flex items-center justify-between gap-3 py-2">
            <label htmlFor="field-eaves" className="text-sm font-medium whitespace-nowrap">
              Drip Edge / Eaves (lf)
            </label>
            <input
              id="field-eaves"
          ...
          <div className="flex items-center justify-between gap-3 py-2">
            <label htmlFor="field-sidewalls" className="text-sm font-medium whitespace-nowrap">
              Sidewall (lf)
            </label>
            <input
              id="field-sidewalls"
          ...
          <div className="flex items-center justify-between gap-3 py-2">
            <label htmlFor="field-endwalls" className="text-sm font-medium whitespace-nowrap">
              Endwall (lf)
            </label>
            <input
              id="field-endwalls"
          ...
          <div className="flex items-center justify-between gap-3 py-2">
            <label htmlFor="field-transitions" className="text-sm font-medium whitespace-nowrap">
              Transition (lf)
            </label>
            <input
              id="field-transitions"
              type="number"
              inputMode="decimal"
              className="border p-2 rounded w-48 h-10 max-w-[60vw]"
      ...
      {/* Notes */}
      <div className="mt-4">
        <label htmlFor="field-notes" className="text-sm font-medium block mb-1">
          Notes
        </label>
        <textarea
          id="field-notes"
        ...
        {[result24, result26].map((res) => (
          <div key={res.gauge} className="bg-white border rounded-2xl p-4">
            ...
            <div className="text-sm text-slate-600">
              Panels: {Number.isFinite(res?.panelLF) ? res.panelLF.toFixed(0) : 0} lf
            </div>
            {res && res.lines ? (
                    ...
                    .map((ln, idx) => (
                      <tr key={idx} className="border-b">
                        ...
                        <td className="py-1 text-right">{`${
                          ln.qty % 1 === 0 ? ln.qty : Math.round(ln.qty)
                        } ${ln.unit || ""}`} × {fmt(ln.price)}</td>
              ...
              <div className="text-sm text-slate-500 mt-2">
                Enter measurements to see pricing.
              </div>
            )}
            {res && (
      ...
      {isPreviewOpen && (
        ...
        >
          <div
            className="bg-white w-[95vw] max-w-5xl rounded-xl shadow-xl overflow-hidden"
